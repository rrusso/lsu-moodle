...
abstract class TurningTechSoapService {
     ...
     * @return MoodleUser
     */
    protected function authenticateuser($TurningPointUserToken) {
        if ($user = $this->service->getuserbyaesauth($TurningPointUserToken)) {
            $this->user = $user;
            return $user;
        }
    ...
    $modifiedstrng = "bnzy3q0KupgwA4Ul6oBiWf3zYEI1b1tG".$request->encryptedUserId;        
    $shaString = hash("sha256", $modifiedstrng, true);
    $encstring = base64_encode($shaString);
	if($encstring == $request->encryptedPassword) {
    	return $this->authenticateuser($request->encryptedUserId);
		} else {
			return;
		}
	}
